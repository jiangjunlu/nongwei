...
WebAboutContent=left(ClearHtml(rs("content")),220)
...
sql="select top 9 [title],file_path,[time] from [en_article] where ArticleType=1 and cid='"&ItemID&"' and pid='"&rs("id")&"' and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
do while not rsp.eof 
rs_url=""
rs_url=ArticleContent_FolderName&"/"&rsp("file_path")
WebNewNewsList=WebNewNewsList&"<div class='DivLi'><span>"&year(rsp("time"))&"/"&month(rsp("time"))&"/"&day(rsp("time"))&"</span> <a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'>"&left(rsp("title"),44)&"</a></div>"
rsp.movenext
loop
end if 
rsp.close
...
'文章栏目列表
set rs=server.createobject("adodb.recordset")
sql="select top 3 [name],[folder],[id],[pid],[ppid] from [en_category] where id<>"&ItemID&" and ClassType=1 and ppid=1 order by [order] "
rs.open(sql),cn,1,1
if not rs.eof then
for i=1 to 3
...
set rsp=server.createobject("adodb.recordset")
sql="select top 9 [title],file_path,[time] from [en_article] where ArticleType=1 and cid='"&rs("id")&"' and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
do while not rsp.eof 
rs_url=""
rs_url=ArticleContent_FolderName&"/"&rsp("file_path")
WebArticleList=WebArticleList&"<div class='DivLi'><span>"&year(rsp("time"))&"/"&month(rsp("time"))&"/"&day(rsp("time"))&"</span> <a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'>"&left(rsp("title"),44)&"</a></div>"
rsp.movenext
loop
end if 
rsp.close
...
set rsp=server.createobject("adodb.recordset")
sql="select top 10 [title],file_path,[image] from [en_article] where ArticleType=2 and pid='"&rs("id")&"' and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
do while not rsp.eof 
rs_url=""
...
'推荐产品
set rsp=server.createobject("adodb.recordset")
sql="select top 9 [title],file_path,[image] from [en_article] where ArticleType=2 and cid='"&ItemID&"' and index_push=1 and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
do while not rsp.eof 
rs_url=""
rs_url=ProductContent_FolderName&"/"&rsp("file_path")
WebPushProductList=WebPushProductList&"<div class='DivLi'>· <a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'>"&left(rsp("title"),30)&"</a></div>"
rsp.movenext
loop
else
WebPushProductList=WebPushProductList&"No hot product."
end if 
rsp.close
set rsp=nothing
...
'推荐咨询
set rsp=server.createobject("adodb.recordset")
sql="select top 9 [title],file_path,[image] from [en_article] where ArticleType=1 and index_push=1  and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
do while not rsp.eof 
rs_url=""
rs_url=ArticleContent_FolderName&"/"&rsp("file_path")
WebPushNewsList=WebPushNewsList&"<div class='DivLi'>· <a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'>"&left(rsp("title"),30)&"</a></div>"
rsp.movenext
loop
else
WebPushNewsList=WebPushNewsList&"No hot news."
end if 
rsp.close
set rsp=nothing
...
set rsp=server.createobject("adodb.recordset")
sql="select top 4 [title],file_path,[image] from [en_article] where ArticleType=3 and cid='"&ItemID&"' and view_yes=1 order by [time] desc"
rsp.open(sql),cn,1,1
if not rsp.eof then
for i=1 to 4
rs_url=""
...
WebCase_List=WebCase_List&"<div class='BlockTitle'><A   href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'>"&left(rsp("title"),26)&"</a></div>"
